...
The system SHALL allow users to permanently delete a local repository from the filesystem via the UI. Before deletion, the system SHALL check for uncommitted changes (excluding files matched by .gitignore, global gitignore, and .git/info/exclude) or unpushed commits and warn the user.
...
- **AND** the warning clearly indicates the presence of:
  - Uncommitted changes (unstaged/staged files, excluding ignored files).
